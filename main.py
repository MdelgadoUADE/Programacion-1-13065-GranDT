...
def menu_torneo(fixture):
    while True:
        print("\n=== Menú de Torneo ===")
        print("1. Jugar próxima fecha") #Esperar eventos
        print("2. Ver fixture")
        print("3. Atras") # esperar
        opcion = input("Elegí una opción: ")

        if opcion == "1":
            menu_torneo()
            """ultimas_jugadas = [f for f in fechas if f["jugada"]]
            if ultimas_jugadas:
                mostrar_fecha(ultimas_jugadas[-1])
            else:
                print("Aún no se jugó ninguna fecha.")"""
        elif opcion == "2":
            ver_fixture(fixture)
        elif opcion == "3":
            print("Saliendo del torneo...")
            break # Deberia volver parea atras OSEA LLAMAR LA FUNCION MENU PRINCIPAL
        else:
